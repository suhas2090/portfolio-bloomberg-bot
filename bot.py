...
# ══════════════════════════════════════════════════════════
# CONFIGURATION  ← Your settings come from env variables
# ══════════════════════════════════════════════════════════

TELEGRAM_TOKEN  = os.environ.get("TELEGRAM_TOKEN", "")
GEMINI_API_KEY  = os.environ.get("GEMINI_API_KEY", "")
ALLOWED_CHAT_ID = os.environ.get("ALLOWED_CHAT_ID", "")  # Your personal chat ID - security lock
RENDER_URL      = os.environ.get("RENDER_URL", "")       # e.g. https://yourapp.onrender.com

# Portfolio - update quantities and avg prices to match yours
PORTFOLIO = {
    "GENUSPOWER": {
...
def is_authorised(update: Update) -> bool:
    """Only respond to YOUR chat - security gate."""
...
def e_pri(p):
    return {"HIGH": "🚨", "MEDIUM": "🟡", "LOW": "ℹ️"}.get(p, "⚪")

def e_imp(i):
    return {"POSITIVE": "🟢", "NEGATIVE": "🔴", "NEUTRAL": "⚪", "MIXED": "🟡"}.get(i, "⚪")

def portfolio_context_str() -> str:
    lines = []
    for s, d in PORTFOLIO.items():
        lines.append(
            f"  {s}: {d['name']} | {d['industry']} | "
            f"Avg ₹{d['avg_price']} × {d['qty']} shares"
        )
    return "\n".join(lines)
...
def cmd_prices(update: Update, context: CallbackContext):
    ...
    for sym, info in PORTFOLIO.items():
        ...
        msg += (
            ...
            f"   P&amp;L: {pnl_e} ₹{pnl:+,.0f} ({pnl_pct:+.1f}%)\n\n"
        )

    total_pnl = total_current - total_invested
    total_pct = (total_pnl / total_invested * 100) if total_invested else 0
    pnl_e = e_chg(total_pnl)

    msg += "─" * 30 + "\n"
    msg += f"💼 <b>TOTAL INVESTED:</b>  ₹{total_invested:,.0f}\n"
    msg += f"📈 <b>CURRENT VALUE:</b>   ₹{total_current:,.0f}\n"
    msg += f"{pnl_e} <b>TOTAL P&amp;L:</b>       ₹{total_pnl:+,.0f} ({total_pct:+.1f}%)\n"
...
def cmd_filings(update: Update, context: CallbackContext):
    ...
    for sym, info in PORTFOLIO.items():
        ...
        for item in news[:2]:
            title = item["title"]
            prompt = f"""
Classify this Indian stock filing for a retail investor.
...
"""
            ...
            exp = data.get("explanation", title)
            act = data.get("action", "")
...
def cmd_news(update: Update, context: CallbackContext):
    ...
    DISRUPTION_KW = [
        "disruption","delay","halt","crisis","shortage","problem",
    ...
    for entity in SUPPLY_CHAIN_WATCHLIST[:6]:
        ...
        for item in items:
            ...
            if any(kw in title_low for kw in DISRUPTION_KW):
                prompt = f"""
Indian stock analyst. Does this news affect any of these stocks?
...
"""
    ...
    # Sector news per stock
    msg += "─" * 30 + "\n"
    msg += "🏭 <b>SECTOR HEADLINES</b>\n\n"
    for sym, info in list(PORTFOLIO.items())[:3]:  # Top 3 to keep message manageable
...
def cmd_analysis(update: Update, context: CallbackContext):
    ...
    for sym, info in PORTFOLIO.items():
        ...
        prompt = f"""
Senior Indian equity analyst. Give buy/hold/sell recommendation:
...
"""
        ...
        e      = ACTION_EMOJI.get(action, "⚪")
        pnl_e  = e_chg(pnl_pct)

        msg += (
            f"{e} <b>{sym}</b> — {action} [{conf}]\n"
            f"   {pnl_e} Price: ₹{cur}  |  P&amp;L: {pnl_pct:+.1f}%\n"
...
def cmd_macro(update: Update, context: CallbackContext):
    ...
    portfolio_str = "\n".join(
        f"  {s}: {i['name']} ({i['industry']})"
        for s, i in PORTFOLIO.items()
    )

    prompt = f"""
Senior Indian equity analyst. Analyse macro/political events for a retail investor's portfolio.
...
def cmd_weekly(update: Update, context: CallbackContext):
    ...
    for sym, info in PORTFOLIO.items():
        ...
        prompt = f"""
Write a weekly briefing for Indian retail investor about {info['name']} ({sym}).
...
def cmd_broker(update: Update, context: CallbackContext):
    ...
    for sym, info in PORTFOLIO.items():
        ...
        prompt = f"""
Extract broker recommendations and concall highlights for {info['name']} ({sym}).
...
If data unavailable for any field use empty array/null. Be honest.
"""
        raw = ask_gemini(prompt)
        try:
            m = re.search(r'\{.*?\}', raw, re.DOTALL)
        ...
        ce = CONSENSUS_EMOJI.get(consensus, "⚪")

        msg += f"{ce} <b>{sym}</b> — Consensus: {consensus}\n"
        ...
        points = d.get("concall_points", [])
        if points:
            msg += "🎙️ Concall highlights:\n"
            for pt in points[:3]:
                msg += f"   • {pt[:100]}\n"

        guidance = d.get("guidance", "")
        if guidance:
            msg += f"👔 Guidance: {guidance[:150]}\n"
...
def cmd_portfolio(update: Update, context: CallbackContext):
    ...
    msg += "─" * 30 + "\n"
    msg += f"💼 <b>Total Invested: ₹{total:,.0f}</b>\n\n"
    msg += "Use /prices for live P&amp;L"
...
def cmd_alert(update: Update, context: CallbackContext):
    ...
    update.message.reply_text(
        "🔔 Running full portfolio scan...\nThis will take 2–3 minutes. "
        "I'll send each section as it's ready."
    )
    cmd_prices(update, context)
    time.sleep(1)
    cmd_filings(update, context)
    time.sleep(1)
    cmd_news(update, context)
    time.sleep(1)
    cmd_macro(update, context)
    time.sleep(1)
    cmd_analysis(update, context)
    update.message.reply_text(
        "✅ <b>Full scan complete!</b>\n\n"
        "Use /weekly for the detailed industry report\n"
...
def handle_text(update: Update, context: CallbackContext):
    """Handle free-form text messages as AI questions."""
    if not is_authorised(update):
        return auth_fail(update)
...
def _handle_question(update: Update, context: CallbackContext, question: str):
    """Core AI Q&A with conversation memory per chat."""
...
You are ONLY knowledgeable about Indian markets (NSE/BSE), SEBI regulations, RBI policy, Indian economy.

THEIR PORTFOLIO:
{portfolio_context_str()}

SUPPLY CHAIN ENTITIES MONITORED: {', '.join(SUPPLY_CHAIN_WATCHLIST[:6])}
...
- If asked about a stock not in their portfolio, you can still answer generally
"""
    ...
    answer = ask_gemini(conv)

    # Store in history
    history.append({"role": "user", "content": question})
    history.append({"role": "assistant", "content": answer})
    _chat_sessions[chat_id] = history[-10:]  # Keep last 5 exchanges

    safe_send(context, update.effective_chat.id, f"🤖 {answer}")


# ══════════════════════════════════════════════════════════
# SCHEDULED JOBS (run automatically every day)
# ══════════════════════════════════════════════════════════

def scheduled_price_alert(context: CallbackContext):
    """Auto-send price update at market close (3:45 PM IST)."""
    if not ALLOWED_CHAT_ID:
        return
    ...
    try:
        ...
        for sym, info in PORTFOLIO.items():
            ...
            e = e_chg(p["day_pct"])
            msg += f"{e} <b>{sym}</b>: ₹{cur} ({p['day_pct']:+.2f}%) | P&amp;L: {pnl_pct:+.1f}%\n"
...
def scheduled_filing_check(context: CallbackContext):
    """Check for HIGH priority filings every 4 hours."""
    if not ALLOWED_CHAT_ID:
        return
    ...
    for sym, info in PORTFOLIO.items():
        ...
        for item in news[:1]:
            prompt = f"""
Classify this Indian stock filing. Company: {info['name']} ({sym})
Filing: "{item['title']}"
...
def home():
    return "🏦 Portfolio Bot is running!", 200
...
def main():
    ...
    # Free-text handler (AI conversation)
    dp.add_handler(MessageHandler(Filters.text & ~Filters.command, handle_text))

    # ── Scheduled jobs (IST = UTC+5:30) ─────────────────
    # Daily price alert at 3:45 PM IST = 10:15 UTC
    jq.run_daily(
        scheduled_price_alert,
        time=datetime.time(hour=10, minute=15, second=0),
        days=(0, 1, 2, 3, 4)  # Mon–Fri only
    )

    # Filing check every 4 hours on weekdays
    jq.run_repeating(
        scheduled_filing_check,
        interval=14400,   # 4 hours in seconds
        first=60          # First run 1 min after start
    )

    log.info("🚀 Portfolio Bot started! Listening for messages...")
